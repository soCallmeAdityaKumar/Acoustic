...
class SeveralArtistsUseCases @Inject constructor (
    private val repository: SpotifyRepository
){
    @RequiresExtension(extension = Build.VERSION_CODES.S, version = 7)
    operator fun invoke(token:String,id:List<String>) : Flow<Resource<Artists>> = flow {
        try{
            emit(Resource.Loading(null))
            val artists=repository.getSeveralArtists("Bearer $token",id)
