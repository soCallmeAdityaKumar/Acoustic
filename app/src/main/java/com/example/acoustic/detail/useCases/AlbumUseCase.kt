...
class AlbumUseCase @Inject constructor (
    ...
    operator fun invoke(token:String,id:String) : Flow<Resource<DetailModel>> = flow {
        try{
            ...
            val album=repository.getForAlbum("Bearer $token",id)
            Log.d("getAlbum",album.toString())
            val detailAlbum=album.toDetailModel()
            Log.d("getAlbum",detailAlbum.toString())
            emit(Resource.Success(detailAlbum))
